...
1. Есть файл с бинарными данными, надо их прочитать, десериализовать (unpack), сериализовать (encode_json), записать в файл. Так же важно следить за тем, что бы скрипт был запущен одновременно только 1 раз  

1. Читаем ввод (STDIN), пишем всё в файл, перехватываем сигнали (INT) с клавиатуры, просим подтверджение выхода из программы повторным нажатием на Ctrl+C, при повторном нажатии выходим из программы. Завершение ввода с клавиатуры (Ctrl+D) тоже приводит к завершению программы. При завершении программы пишем статистику (размер прочитанных данных, количество строк, стредняя длинна строки). Имя файла для записи должно задаваться при запуске скрипта параметром

1. Реализовать параллельное умножение квадратных матриц. Количество потоков задаётся параметром. Исходная матрица задаётся на STDIN, результирующая пишется в STDOUT. Вход и выход в формате L(L*). Где первое число - это размер матрицы, остальные числа - это значения соответствующих ячеек (не забываем, что на входе ячеек в 2 раза больше).
