...
Требуется реализовать набор классов `DBI::ActiveRecord`, позволяющих легко и непринужденно создавать объекты интегрированные с таблицами базы данных.
...
* понять в каких местах требуется добавить реализацию недостающей функциональности
* реализовать его
...
4. Работа с первичным ключом. У каждой таблицы обязательно должен быть определен первчиный ключ (primary key), при чем только один. В классе описывается параметром `index => 'primaty'` атрибута. 
