...
| `--sort band|year|album|track|format` | Сортировать по возрастанию значения указанного параметра |
| `-columns col1,col2,...,colN` | Список колонок через запятую, которые должны появиться в таблице (с учетом порядка). Возможные варианты: `band|year|album|track|format`. Дублирование допускается. Опциональный параметр, при отсутствии использовать значение по умолчанию. |
